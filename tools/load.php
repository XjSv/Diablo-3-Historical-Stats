...
if($test_mode) {
    echo "Running in test mode.<br><br>";
    $db_name       = DEV_DB;
    $user_profiles = $dev_user_profiles;
}
        ...
        if(is_array($CAREER_DATA)) {
            ...
            foreach($HERO_DATA as $key => $hero) {
                if(is_array($hero)) {
                    ...
                    //
                    $last_hero_insert = $hero_collection->find(array('id'=> $hero['id']))->sort(array('last-updated' => -1))->limit(1);
                    $last_hero_date   = null;
